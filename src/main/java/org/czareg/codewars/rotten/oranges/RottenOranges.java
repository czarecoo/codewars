...
public class RottenOranges {
    ...
    public static int calcRotTime(int[][] orangeBox) {
        int timeUnits = 0;
        BoxState previousBoxState;
        BoxState currentBoxState = new BoxState(orangeBox);
        for (; ; ) {
            previousBoxState = currentBoxState;
            currentBoxState = currentBoxState.passTime();
            if (previousBoxState.equals(currentBoxState)) {
                return currentBoxState.hasFresh() ? -1 : timeUnits;
            }
            timeUnits++;
        }
    }
