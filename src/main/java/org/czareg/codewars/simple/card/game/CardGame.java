...
public class CardGame {
    ...
    private static final String WINNER_TEMPLATE = "%s wins %d to %d";

    public String winner(String[] deckSteve, String[] deckJosh) {
        int steveScore = 0;
        int joshScore = 0;
        for (int i = 0; i < deckSteve.length; i++) {
            if (valueOf(deckSteve[i]) > valueOf(deckJosh[i])) {
                steveScore++;
            } else if (valueOf(deckSteve[i]) < valueOf(deckJosh[i])) {
                joshScore++;
            }
        }
        return result(steveScore, joshScore);
    }
    ...
    private String result(int steveScore, int joshScore) {
        if (steveScore > joshScore) {
            return WINNER_TEMPLATE.formatted("Steve", steveScore, joshScore);
        } else if (steveScore < joshScore) {
            return WINNER_TEMPLATE.formatted("Josh", joshScore, steveScore);
        } else {
            return "Tie";
        }
    }
