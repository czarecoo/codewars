...
import static org.czareg.codewars.good.vs.evil.GoodVsEvil.Result.*;
...
public class GoodVsEvil {
    ...
    private static final int[] GOOD_VALUES = {1, 2, 3, 3, 4, 10};
    private static final int[] EVIL_VALUES = {1, 2, 2, 2, 3, 5, 10};

    public static String battle(String goodAmounts, String evilAmounts) {
        int goodValue = calculateValue(goodAmounts, GOOD_VALUES);
        int evilValue = calculateValue(evilAmounts, EVIL_VALUES);
        if (goodValue == evilValue) {
            return TIE.getText();
        }
        return goodValue > evilValue ? GOOD_WON.getText() : EVIL_WON.getText();
    }

    private static int calculateValue(String input, int[] values) {
        String[] amounts = input.split(" ");
        int value = 0;
        for (int i = 0; i < amounts.length; i++) {
            value += Integer.parseInt(amounts[i]) * values[i];
        }
        return value;
    }
    ...
    enum Result {
        ...
        GOOD_WON("Battle Result: Good triumphs over Evil"),
        EVIL_WON("Battle Result: Evil eradicates all trace of Good"),
