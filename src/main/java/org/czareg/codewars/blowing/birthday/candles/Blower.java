...
These candles need to be blown a certain number of times before they're finally extinguished, and those numbers on the candles are the required times to blow them.
...
class Blower {
    ...
    private static boolean atLeastOneCandleIsLit(int[] candles) {
        return Arrays.stream(candles).anyMatch(candle -> candle != 0);
    }

    private static void blow(int[] candles) {
        int firstLitCandleIndex = findFirstLitCandle(candles);
        for (int i = 0; i < CANDLES_PER_BLOW; i++) {
            reduceCandlePowerByOneIfPossible(candles, firstLitCandleIndex + i);
        }
    }

    private static int findFirstLitCandle(int[] candles) {
        for (int i = 0; i < candles.length; i++) {
            if (candles[i] > 0) {
                return i;
            }
        }
        throw new IllegalStateException("No lit candle found.");
    }

    private static void reduceCandlePowerByOneIfPossible(int[] candles, int index) {
        if (index >= candles.length) {
            return;
        }
        int candle = candles[index];
        if (candle <= 0) {
            return;
        }
        candles[index]--;
