...
class PresentsCounter {
    ...
    static int howManyGifts(int budget, List<Integer> giftCosts) {
        def sortedGiftCosts = giftCosts.sort(false)
        def totalGifts = 0
        def totalCost = 0
        for (giftCost in sortedGiftCosts) {
            if (totalCost + giftCost <= budget) {
                totalCost += giftCost
                totalGifts++
            }
        }
